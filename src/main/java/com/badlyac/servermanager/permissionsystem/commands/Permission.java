...
public class Permission {
    ...
    private static int setPermission(CommandContext<CommandSourceStack> context, String playerSelector, String permission, boolean value) {
        ...
        if (players.isEmpty()) {
            ColoredMsg.sendToCommandSender(context.getSource(),"No players found for the selector: " + playerSelector, ChatFormatting.RED);
            return 0;
        }
        ...
        for (ServerPlayer player : players) {
            ...
            ColoredMsg.sendToCommandSender(context.getSource(),"Permission " + permission + " set to " + value + " for player " + player.getName().getString(), ChatFormatting.GREEN);;
        }
        return 1;
    }
    ...
    private static int removePermission(CommandContext<CommandSourceStack> context, String playerSelector, String permission) {
        ...
        for (ServerPlayer player : players) {
            ...
            ColoredMsg.sendToCommandSender(context.getSource(), "Permission " + permission + " removed from player " + player.getName().getString(), ChatFormatting.GREEN);
        }
        return 1;
    }
